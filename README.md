...
- **Статистика посещений** прямо на сайте (кнопка «Статистика»): визиты,
  ...
  интерфейса.
- **Географическая подсказка**: если идиом не найден, сайт спрашивает геокодер
  [Nominatim](https://nominatim.openstreetmap.org) (бесплатно, без ключа), не
  является ли запрос названием места, и показывает языки этого района. Так
  «sandonatese» приводит к Сан-Донà-ди-Пьяве и венетскому, хотя такого
  языкоида нет ни в одном каталоге. Подсказка появляется, только когда название
  места и запрос имеют общую основу — случайные совпадения по улицам отсеиваются.
